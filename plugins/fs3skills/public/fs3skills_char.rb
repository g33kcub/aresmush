module AresMUSH
  class Character
    ...
    collection :fs3_sorcery, "AresMUSH::FS3Sorcery"

    before_delete :delete_abilities

    def delete_abilities
      [ self.fs3_attributes, self.fs3_action_skills, self.fs3_background_skills, self.fs3_languages, self.fs3_advantages, self.fs3_sorcery ].each do |list|
        list.each do |a|
          a.delete
        end
      end
