  ...
  module FS3Skills
    ...
    def self.set_ability(char, ability_name, rating)
      ...
      if (ability)
        ...
        case ability_type
        when :sorcery
          ability = FS3Sorcery.create(character: char, name: ability_name, rating: 1)
        when :action
      ...
      if (rating == min_rating)
        if (ability && (ability_type == :background || ability_type == :language || ability_type == :advantage || ability_type == :sorcery))
          ability.delete
        end
      end
    ...
    def self.get_min_rating(ability_type)
      ...
      when :background, :language, :advantage, :sorcery
        min_rating = 0
      when :attribute
        min_rating = 1
      end
    ...
    def self.reset_char(char)
      ...
      char.fs3_sorcery.each { |s| s.delete }
